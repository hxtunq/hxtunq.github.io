...
export default function ActivityHeatmap({
  ...
  // Dynamic theme-adaptive contribution color classes
  const getColorClass = (count: number, isFuture: boolean, isCurrentYear: boolean) => {
    if (!isCurrentYear || isFuture) {
      return "opacity-0 pointer-events-none";
    }
    if (count === 0) {
      return "bg-[var(--heatmap-empty)] border border-[var(--heatmap-empty-border)] hover:ring-1 hover:ring-brand-primary";
    }
    if (count === 1) {
      return "bg-[var(--heatmap-l1)] border border-[var(--heatmap-l1-border)] hover:ring-1 hover:ring-brand-primary";
    }
    if (count === 2) {
      return "bg-[var(--heatmap-l2)] border border-[var(--heatmap-l2-border)] hover:ring-1 hover:ring-brand-primary";
    }
    if (count === 3) {
      return "bg-[var(--heatmap-l3)] border border-[var(--heatmap-l3-border)] hover:ring-1 hover:ring-brand-primary";
    }
    return "bg-[var(--heatmap-l4)] border border-[var(--heatmap-l4-border)] hover:ring-1 hover:ring-brand-primary";
  };
  ...
  return (
    <div className="w-full text-left font-sans">
      {/* Container holding both Motivation (Left) & Recent Activity (Right) */}
      <div className="flex flex-col lg:flex-row gap-6 lg:gap-4 items-stretch w-full">
        {/* Left Column: My Motivation */}
        <div className="flex-1 min-w-0 flex flex-col">
          {leftHeaderSlot && (
            <div className="mb-3 px-0.5 flex items-center min-h-[28px]">
              {leftHeaderSlot}
            </div>
          )}
          {leftSlot}
        ...
        {/* Right Column: Recent Website Activity */}
        <div className="w-full lg:w-[670px] lg:shrink-0 flex flex-col">
          <div className="flex items-center justify-between gap-3 mb-3 px-0.5 min-h-[28px]">
          ...
          <div className="flex-1 bg-brand-surface-lowest border border-brand-surface-highest rounded-md px-3.5 sm:px-4 py-2.5 sm:py-3 shadow-sm flex flex-col justify-between">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mt-3 pt-2.5 border-t border-brand-surface-highest/60 text-[11px] font-sans text-brand-on-surface-variant/80">
              ...
              <div className="flex items-center gap-1.5 font-mono text-[10px]">
                <span>Less</span>
                <div className="w-[10px] h-[10px] rounded-[2px] bg-[var(--heatmap-empty)] border border-[var(--heatmap-empty-border)]" />
                <div className="w-[10px] h-[10px] rounded-[2px] bg-[var(--heatmap-l1)] border border-[var(--heatmap-l1-border)]" />
                <div className="w-[10px] h-[10px] rounded-[2px] bg-[var(--heatmap-l2)] border border-[var(--heatmap-l2-border)]" />
                <div className="w-[10px] h-[10px] rounded-[2px] bg-[var(--heatmap-l3)] border border-[var(--heatmap-l3-border)]" />
                <div className="w-[10px] h-[10px] rounded-[2px] bg-[var(--heatmap-l4)] border border-[var(--heatmap-l4-border)]" />
                <span>More</span>
              </div>
            </div>
          </div>
